...
## Auto index template (write markdown, drawio, xmind)

You can keep writing `.md`, `.drawio`, and `.xmind` files. This repo includes an auto-index template:
...
- If a directory has markdown notes or diagrams (`.drawio` / `.xmind`) and no `index.md`, it will create one.
...
- XMind links shown on pages are embedded automatically via xmind-embed-viewer.
...
- Trigger: daily schedule + manual run (`workflow_dispatch`)
